            ...
            <div class="-my-2 -mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
                <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
                    <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                        <table class="min-w-full divide-y px-5  divide-gray-300">
                                ...
                                @foreach ($categories as $category)
                                    <tr>
                                        <td
                                            ...
                                            {{ ++$i }}</td>
                                        <td class="whitespace-nowrap px-3 py-4 text-lg font-semibold text-black">
                                            {{ $category->name }}
                                        </td>
                                        ...
                                        <td
                                            ...
                                            @can('category-edit')
                                                <a href="{{ route('categories.edit', $category->id) }}"
                                                    class="  bg-green-400 mx-2 py-2 rounded-md shadow-sm text-white hover:text-indigo-900 text-sm px-5 ">Edit</a>
                                            @endcan
                                            @can('category-delete')
                                                <form id="delete_form" action="{{ route('categories.destroy', $category->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
